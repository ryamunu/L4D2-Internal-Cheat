...
class IVRenderView {
public:
  virtual void DrawBrushModel(IClientEntity *baseentity, void *model,
  ...
  virtual void GetVisibleFogVolume(const Vector &eyePoint,
                                   const void *pVisOverrideData,
                                   void *pInfo) = 0;
  virtual void *CreateWorldList() = 0;
  virtual void BuildWorldLists(void *pList, void *pInfo, int iForceFViewLeaf,
                               const void *pVisData = NULL,
                               bool bShadowDepth = false,
                               float *pReflectionWaterHeight = NULL) = 0;
  virtual void DrawWorldLists(void *pRenderContext, void *pList,
...
public:
  ...
  virtual void DrawTranslucentSurfaces(void *pRenderContext, void *pList,
                                       int *pSortList, int sortCount,
  ...
  virtual void ViewDrawFade(byte *color, IMaterial *pMaterial,
                            bool mapFullTextureToScreen = true) = 0;
  ...
  virtual bool LeafContainsTranslucentSurfaces(void *pList, int sortIndex,
                                               unsigned long flags) = 0;
  virtual bool DoesBoxIntersectWaterVolume(const Vector &mins,
                                           const Vector &maxs,
                                           int leafWaterDataID) = 0;
  virtual void SetAreaState(unsigned char *chAreaBits,
                            unsigned char *chAreaPortalBits) = 0;
  virtual void VGui_Paint(int mode) = 0;
  virtual void Push3DView(IMatRenderContext *pRenderContext, const void *view,
                          int nFlags, ITexture *pRenderTarget,
                          void *frustumPlanes) = 0;
  virtual void Push2DView(IMatRenderContext *pRenderContext, const void *view,
                          int nFlags, ITexture *pRenderTarget,
                          void *frustumPlanes) = 0;
  virtual void PopView(IMatRenderContext *pRenderContext,
                       void *frustumPlanes) = 0;
  virtual void SetMainView(const Vector &vecOrigin, const Vector &angles) = 0;
  virtual void ViewSetupVisEx(bool novis, int numorigins, const Vector origin[],
                              unsigned int &returnFlags) = 0;
  virtual void OverrideViewFrustum(void *custom) = 0;
  virtual void DrawBrushModelShadowDepth(IClientEntity *baseentity, void *model,
                                         const Vector &origin,
                                         const Vector &angles,
                                         ERenderDepthMode_t DepthMode) = 0;
  virtual void UpdateBrushModelLightmap(void *model,
                                        IClientRenderable *pRenderable) = 0;
  virtual void BeginUpdateLightmaps(void) = 0;
  virtual void EndUpdateLightmaps() = 0;
  virtual void OLD_SetOffCenterProjectionMatrix(float fov, float zNear,
                                                float zFar, float flAspectRatio,
                                                float flBottom, float flTop,
                                                float flLeft,
                                                float flRight) = 0;
  virtual void OLD_SetProjectionMatrixOrtho(float left, float top, float right,
                                            float bottom, float zNear,
                                            float zFar) = 0;
  virtual void Push3DView(const void *pSetup, int unk1, ITexture *unk2,
                          void *pPlane, ITexture *unk3) = 0;
  virtual void GetMatricesForView(const void *view, VMatrix *pWorldToView,
