...
public:
  ...
  virtual void DrawColoredCircle(int centerx, int centery, float radius, int r,
                                 int g, int b, int a) = 0;
  virtual int DrawColoredText(HFont font, int x, int y, int r, int g, int b,
                              int a, char *fmt, ...) = 0;
  virtual void DrawColoredTextRect(HFont font, int x, int y, int w, int h,
                                   int r, int g, int b, int a, char *fmt,
                                   ...) = 0;
  virtual void DrawTextHeight(HFont font, int w, int &h, char *fmt, ...) = 0;
  virtual int DrawTextLen(HFont font, char *fmt, ...) = 0;
  virtual void DrawPanelIn3DSpace(VPANEL pRootPanel,
                                  const VMatrix &panelCenterToWorld,
                                  int nPixelWidth, int nPixelHeight,
                                  float flWorldWidth, float flWorldHeight) = 0;
...
public:
  ...
  virtual void DrawColoredTextMultiLine(unsigned long unk1, int unk2, int unk3,
                                        int unk4, int unk5, int unk6, int unk7,
                                        int unk8, char *unk9, ...) = 0;

public:
  inline void StartDrawing() {
    reinterpret_cast<void(__thiscall *)(void *)>(U::Offsets.m_dwStartDrawing)(
        this);
  }

  inline void FinishDrawing() {
    reinterpret_cast<void(__thiscall *)(void *)>(U::Offsets.m_dwFinishDrawing)(
        this);
  }
};
