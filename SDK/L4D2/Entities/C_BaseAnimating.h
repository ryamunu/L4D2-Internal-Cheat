...
public:
  enum { NUM_POSEPAREMETERS = 24, NUM_BONECTRLS = 4, NUM_STUDIOBONES = 128 };
...
public:
  ...
  virtual void GetPoseParameters(CStudioHdr *pStudioHdr,
                                 float poseParameter[NUM_POSEPAREMETERS]) = 0;
  virtual void BuildTransformations(CStudioHdr *pStudioHdr, void *pos, void *q,
  ...
  virtual int InternalDrawModel(int flags,
                                const RenderableInstance_t &instance) = 0;
  ...
  virtual void FireEvent(const Vector &origin, const Vector &angles, int event,
                         const char *options) = 0;
  virtual void FireObsoleteEvent(const Vector &origin, const Vector &angles,
                                 int event, const char *options) = 0;
  virtual bool DispatchMuzzleEffect(const char *options,
                                    bool isFirstPerson) = 0;
  virtual void EjectParticleBrass(const char *pEffectName,
                                  const int iAttachment) = 0;
  virtual void StandardBlendingRules(CStudioHdr *pStudioHdr, void *pos, void *q,
                                     float currentTime, int boneMask) = 0;
  virtual void MaintainSequenceTransitions(void *boneSetup, float flCycle,
                                           void *pos, void *q) = 0;
  virtual void AccumulateLayers(void *boneSetup, void *pos, void *q,
                                float currentTime) = 0;
  ...
  virtual void SaveRagdollInfo(int numbones, const matrix3x4_t &cameraTransform,
                               void *pBoneToWorld) = 0;
  virtual bool RetrieveRagdollInfo(void *pos, void *q) = 0;
  virtual void GetRagdollInitBoneArrays(matrix3x4_t *pDeltaBones0,
  ...
  virtual void FormatViewModelAttachment(int nAttachment,
                                         matrix3x4_t &attachmentToWorld) = 0;
...
public:
  inline bool GetHitboxPositionByName(const char *name, Vector &vPos) {
    ...
    if (!this->SetupBones(Matrix, NUM_STUDIOBONES, 0x100,
    ...
    for (int i = 0; i < pSet->numhitboxes; i++) {
      ...
      mstudiobone_t *pBone = pHdr->pBone(pBox->bone);
      if (!pBone)
        continue;

      // Case-insensitive name check
      const char *pszBoneName = pBone->pszName();
      bool match = true;
      for (int n = 0; name[n]; n++) {
        if (tolower(pszBoneName[n]) != tolower(name[n])) {
          match = false;
          break;
        }
      }

      // Also try simple strstr fallback if not exact match at start
      if (!match && !strstr(pszBoneName, name)) {
        // Deep search
        char boneLower[128];
        strncpy(boneLower, pszBoneName, 127);
        for (int k = 0; boneLower[k]; k++)
          boneLower[k] = tolower(boneLower[k]);

        char searchLower[128];
        strncpy(searchLower, name, 127);
        for (int k = 0; searchLower[k]; k++)
          searchLower[k] = tolower(searchLower[k]);

        if (strstr(boneLower, searchLower)) {
          match = true;
        }
      }

      if (match) {
        U::Math.VectorTransform((pBox->bbmin + pBox->bbmax) * 0.5f,
                                Matrix[pBox->bone], vPos);
        return true;
      }
    }
    return false;
  ...
  inline bool GetHitboxPositionByGroup(const int nGroup, Vector &vPos) {
    ...
    // Gets head properly, possibly fails for other groups due to obvious
    // reasons.
    ...
    if (!pFinalBox)
      return false;
    U::Math.VectorTransform((pFinalBox->bbmin + pFinalBox->bbmax) * 0.5f,
                            Matrix[pFinalBox->bone], vPos);
    return true;
  }

  inline bool GetHitboxPosition(int nHitbox, Vector &vPos) {
    const model_t *pModel = this->GetModel();
    if (!pModel)
      return false;
    ...
    mstudiobbox *pBox = pSet->pHitbox(nHitbox);
    if (!pBox)
      return false;

    matrix3x4_t Matrix[128];
    if (!this->SetupBones(Matrix, 128, 0x100, I::GlobalVars->curtime))
      return false;
  ...
  inline bool GetBonePosition(int nBoneIndex, Vector &vPos) {
    matrix3x4_t Matrix[128];
    if (!this->SetupBones(Matrix, 128, 0x100, I::GlobalVars->curtime))
      return false;

    U::Math.VectorTransform(Vector(0, 0, 0), Matrix[nBoneIndex], vPos);
    return true;
  }
};
...
public:
  ...
  virtual bool StartSceneEvent(CSceneEventInfo *info, CChoreoScene *scene,
                               CChoreoEvent *evnt, CChoreoActor *actor,
  ...
  virtual bool CheckSceneEventCompletion(CSceneEventInfo *info,
                                         float currenttime, CChoreoScene *scene,
                                         CChoreoEvent *evnt) = 0;
