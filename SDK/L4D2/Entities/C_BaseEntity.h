...
#include "../../../Util/NetVarManager/NetVarManager.h"
#include "../../Globals.h"
...
public:
  ...
  virtual bool HandleShotImpactingWater(const void *info, const Vector &vecEnd,
                                        ITraceFilter *pTraceFilter,
                                        Vector *pVecTracerDest) = 0;
  ...
  virtual void DispatchTraceAttack(const CTakeDamageInfo *info,
                                   ...
                                   CDmgAccumulator *pAccumulator = NULL) = 0;
  ...
  virtual bool GetKeyValue(const char *szKeyName, char *szValue,
                           int iMaxLen) = 0;
...
public:
  ...
  virtual bool InitializeAsClientEntity(const char *pszModelName,
                                        int renderGroup) = 0; // RenderGroup_t
  ...
  virtual void AddDecal(const Vector &rayStart, const Vector &rayEnd,
                        ...
                        bool doTrace, trace_t &tr, int maxLODToDecal = 0) = 0;
  ...
  virtual void ImpactTrace(trace_t *pTrace, int iDamageType,
                           const char *pCustomImpactName) = 0;
...
public:
  ...
  virtual bool AttemptToPowerup(int iPowerup, float flTime, float flAmount = 0,
                                C_BaseEntity *pAttacker = NULL,
                                void *pDamageModifier = NULL) = 0;
...
public:
  ...
  virtual void BoneMergeFastCullBloat(Vector &localMins, Vector &localMaxs,
                                      ...
                                      const Vector &thisEntityMaxs) const = 0;
  virtual bool OnPredictedEntityRemove(bool isbeingremoved,
                                       C_BaseEntity *predicted) = 0;
  ...
  virtual void PerformCustomPhysics(Vector *pNewPosition, Vector *pNewVelocity,
                                    Vector *pNewAngles,
                                    Vector *pNewAngVelocity) = 0;
...
public:
  ...
  PhysicsRunThink(thinkmethods_t thinkMethod = THINK_FIRE_ALL_FUNCTIONS) {
    return reinterpret_cast<bool(__thiscall *)(void *, thinkmethods_t)>(
        U::Offsets.m_dwPhysicsRunThink)(this, thinkMethod);
  }

  inline unsigned char &m_MoveType() {
    return *reinterpret_cast<unsigned char *>(reinterpret_cast<DWORD>(this) +
                                              0x144);
  }
  /*
  Jockey = 265,
  ...
  */
  inline bool IsZombie() {
    int id = this->GetClientClass()->m_ClassID;
    return (id == G::ClassID.Jockey || id == G::ClassID.Spitter ||
            id == G::ClassID.Charger || id == G::ClassID.Hunter ||
            id == G::ClassID.Smoker || id == G::ClassID.Boomer ||
            id == G::ClassID.Tank || id == G::ClassID.Infected ||
            id == G::ClassID.Witch);
  }
  ...
  const char *GetSequenceName(int n_Sequence) {
    ...
    static auto get_sequence_name_ =
        reinterpret_cast<get_sequence_name>(U::Offsets.m_dwSequenceName);
    return get_sequence_name_(this, n_Sequence);
  }
  inline bool IsAlive() {
    int id = GetClientClass()->m_ClassID;

    // Common Infected needs special handling (SolidFlags)
    if (id == G::ClassID.Infected) {
      // FSOLID_NOT_SOLID (0x0004) is set on dead common infected
      if (m_usSolidFlags() & 4)
        return false;

      return true; // Usually enough for commons
    }

    // First check health if it's available and virtual which it is
    if (GetHealth() <= 0)
      return false;

    // Players/Specials also have m_lifeState
    if (id == G::ClassID.CTerrorPlayer || id == G::ClassID.SurvivorBot ||
        id == G::ClassID.Tank || id == G::ClassID.Hunter ||
        id == G::ClassID.Smoker || id == G::ClassID.Boomer ||
        id == G::ClassID.Jockey || id == G::ClassID.Spitter ||
        id == G::ClassID.Charger) {
      return (m_lifeStateInfected() == 0); // 0 = LIFE_ALIVE
    }
  ...
  bool IsAlive()
  {
          if (constexpr int FSOLID_NOT_SOLID = 0x0004;
  GetSolidFlags().has_flag(FSOLID_NOT_SOLID)) return false;

          const auto activity = GetCurrentActivity();
          return !((activity >= ActivityList::ACT_DIESIMPLE && activity <=
  ActivityList::ACT_DIERAGDOLL) || activity ==
  ActivityList::ACT_DIE_BARNACLE_SWALLOW
                           || (activity >= ActivityList::ACT_DIE_FRONTSIDE &&
  activity <= ActivityList::ACT_DIE_STANDING)
                           || (activity >=
  ActivityList::ACT_TERROR_DIE_FROM_STAND && activity <=
  ActivityList::ACT_TERROR_DIE_RIGHTWARD_FROM_SHOTGUN));
  }
  ...
  */
  inline bool ValidEntity(int m_Sequence, int m_SolidFlags) {
    ...
    /* IsSolid(this->GetSolidType(), this->GetSolidFlags()) &&
     * !strstr(GetSequenceName(), "Death") && !this->IsBurning() */
    int classindex = GetClientClass()->m_ClassID;
    const char *seqname = GetSequenceName(m_Sequence);
    if (classindex == G::ClassID.Tank) {
      if (!IsAlive() && !(GetHealth() <= GetMaxHealth()))
        return false;
    }
    if (classindex == G::ClassID.Jockey || classindex == G::ClassID.Spitter ||
        classindex == G::ClassID.Charger || classindex == G::ClassID.Hunter ||
        classindex == G::ClassID.Smoker || classindex == G::ClassID.Boomer ||
        classindex == G::ClassID.Tank) {
      if (!IsAlive())
        return false;
    }
    if (classindex == G::ClassID.Infected) {
