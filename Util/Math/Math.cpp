...
void CUtil_Math::VectorTransform(const Vector input, const matrix3x4_t &matrix,
                                 Vector &output) {
  output[0] =
      input.Dot({matrix[0][0], matrix[0][1], matrix[0][2]}) + matrix[0][3];
  output[1] =
      input.Dot({matrix[1][0], matrix[1][1], matrix[1][2]}) + matrix[1][3];
  output[2] =
      input.Dot({matrix[2][0], matrix[2][1], matrix[2][2]}) + matrix[2][3];
}

void CUtil_Math::BuildTransformedBox(Vector *v2, const Vector bbmin,
                                     const Vector bbmax, const matrix3x4_t &m) {
  Vector v[8];
...
void CUtil_Math::PointsFromBox(const Vector mins, const Vector maxs,
                               Vector *points) {
  points[0][0] = mins[0];
  points[0][1] = mins[1];
  points[0][2] = mins[2];
