...
public:
  ...
  float Lenght(void) const { return ::sqrtf(x * x + y * y); }

  float LenghtSqr(void) const { return (x * x + y * y); }

  float DistTo(const Vector2D &v) const { return (*this - v).Lenght(); }

  float DistToSqr(const Vector2D &v) const { return (*this - v).LenghtSqr(); }
