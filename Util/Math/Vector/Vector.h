...
public:
  ...
  Vector operator+(float v) const { return Vector(x + v, y + v, z + v); }
  ...
  float LenghtSqr(void) const { return LengthSqr(); }

  float Normalize() {
    float fl_lenght = Length();
    float fl_lenght_normal =
        1.f / ((1.19209290E-07F) + fl_lenght); // FLT_EPSILON
  ...
  float NormalizeInPlace() { return Normalize(); }

  float Length2D(void) const { return sqrtf(x * x + y * y); }

  float Lenght2D(void) const { return Length2D(); }

  float Length2DSqr(void) const { return (x * x + y * y); }

  float Lenght2DSqr(void) const { return Length2DSqr(); }
  ...
  bool IsZero(const float flScale = 0.0f) const {
    return (x > -flScale && x < flScale && y > -flScale && y < flScale &&
            z > -flScale && z < flScale);
  }

  Vector Scale(float fl) { return Vector(x * fl, y * fl, z * fl); }
...
public:
  ...
  explicit VectorAligned(const Vector &othr) { Init(othr.x, othr.y, othr.z); }
